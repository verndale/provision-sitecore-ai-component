...
- Output is `scripts/graph/data/graph.json` — timestamp-free, sorted, byte-stable — so a rebuild only diffs when the content graph changes. Canonical GitHub PR/issue URLs outside code fences become repo-qualified `githubRefs` metadata on the citing node while legacy number arrays remain; GitHub evidence never becomes a separate node or live viewer request.
...
- `scripts/graph/routing.cjs` + `routing-policy.json` give deterministic navigation (`why`/`wiring`/`impact` intents) — shortest-path over per-edge costs with a hub penalty, never inferring beyond existing edges; `scripts/wiki/navigate.cjs` returns the minimal route plus per-page and total bytes.
- `scripts/graph/serve.cjs` + `viewer/` serve an interactive Sigma.js viewer (`pnpm graph:view`, localhost:4173): node color = type, size = degree, Source/Target controls highlight the shortest route, and repo-qualified GitHub evidence is searchable and rendered with safe links from committed JSON only.
- Evidence queries normalize URL query/fragment decoration while rejecting embedded hosts, malformed or unsafe numbers. Legacy number arrays derive only from those canonical references; merge contexts require normalized string paths, string commit identity fields, PR text, and merge timestamps while retaining explicit legacy aliases; evidence and graph-link extraction respect the opening fence character and length; and issue refresh rejects symlinked topic files, scan directories, and wiki ancestors. Node and browser routing reject malformed exclusion/intent type arrays and share a `0.05` per-KiB penalty so equal-hop routes choose the smaller read budget.
...
- Viewer assets and the routing policy are intentionally not graph nodes; focused source-contract tests cover safe evidence rendering and workflow parity, while browser layout remains a visual check.
