# Connections — Cross-subsystem links

Markdown links whose source and target live in different areas — the seams between subsystems.

Part of the [wiring map](../connections.md), generated from the knowledge graph — **do not edit by hand**. Rebuilt on every `pnpm graph:build` and verified fresh by `pnpm test`.

- [Agent guide](../../AGENTS.md) → [Authoring API contract](../../skills/provision-sitecore-component/references/authoring-api.md) (×2)
- [Agent guide](../../AGENTS.md) → [Confluence spec import](../../skills/provision-sitecore-component/references/confluence-import.md)
- [Agent guide](../../AGENTS.md) → [Component manifest contract](../../skills/provision-sitecore-component/references/manifest-contract.md) (×2)
- [Agent guide](../../AGENTS.md) → [Bounded retry & self-correction contract (shared)](../../skills/provision-sitecore-component/references/retry-contract.md)
- [Agent guide](../../AGENTS.md) → [TSX scaffold contract](../../skills/provision-sitecore-component/references/tsx-template.md)
- [Agent guide](../../AGENTS.md) → [Field type mapping](../../skills/provision-sitecore-component/references/type-mapping.md) (×2)
- [Agent guide](../../AGENTS.md) → [provision-sitecore-component](../../skills/provision-sitecore-component/SKILL.md)
- [Agent guide](../../AGENTS.md) → [Context Wiki](../../wiki/INDEX.md) (×2)
- [Contributing](../../CONTRIBUTING.md) → [Wiki Mechanics](../../wiki/MECHANICS.md)
- [provision-sitecore-component](../../README.md) → [Authoring API contract](../../skills/provision-sitecore-component/references/authoring-api.md)
- [provision-sitecore-component](../../README.md) → [Component manifest contract](../../skills/provision-sitecore-component/references/manifest-contract.md)
- [provision-sitecore-component](../../README.md) → [Field type mapping](../../skills/provision-sitecore-component/references/type-mapping.md)
- [provision-sitecore-component](../../README.md) → [Context Wiki](../../wiki/INDEX.md) (×2)
- [provision-sitecore-component](../../README.md) → [Wiki Mechanics](../../wiki/MECHANICS.md)
- [provision-sitecore-component — operator docs](../../skills/provision-sitecore-component/README.md) → [provision-sitecore-component](../../README.md)
- [Wiki Mechanics](../../wiki/MECHANICS.md) → [build-graph.cjs](../../scripts/graph/build-graph.cjs)
- [Sitecore component provisioning — Design History](../../wiki/topics/sitecore-provisioning.md) → [pretooluse-guard.cjs](../../scripts/hooks/pretooluse-guard.cjs)
- [Sitecore component provisioning — Design History](../../wiki/topics/sitecore-provisioning.md) → [Component manifest contract](../../skills/provision-sitecore-component/references/manifest-contract.md)
- [Sitecore component provisioning — Design History](../../wiki/topics/sitecore-provisioning.md) → [build-plan.cjs](../../src/build-plan.cjs)
- [Sitecore component provisioning — Design History](../../wiki/topics/sitecore-provisioning.md) → [emit-tsx.cjs](../../src/emit-tsx.cjs)
- [Sitecore component provisioning — Design History](../../wiki/topics/sitecore-provisioning.md) → [executor.cjs](../../src/executor.cjs)
