...
✅ Spec lists Droplist choices (any names — Image CTA Row happens to use `light`/`dark`; another spec may use different item names and labels) → copy each exact name, display name, and project value into `optionSource.options`; ask where the project's option item template exists or should be created, its value-field name, `searchRoot`, and an explicit Data-folder `fallback.path`. Source becomes `query:<folder>/*` at check/push time.
...
❌ Spec lists Droplist values → the agent writes `name=Label` into Source, switches the field to Enum/Droplink, or reuses another component's option names (for example always `light`/`dark`) instead of this spec's list.
