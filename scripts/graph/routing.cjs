...
function nodeHaystack(node) {
  const evidence = (node.githubRefs || []).flatMap((ref) => [
    ref.url,
    formatGithubRef(ref),
...
function formatRoute(result) {
  ...
  const lines = ["Route " + result.intent + " · cost " + result.cost + " · " + result.totalBytes + " B to read:"];
  for (const [index, item] of result.itinerary.entries()) {
    lines.push((index + 1) + ". " + item.id + " [" + item.bytes + " B] — " + item.relation + " — authority: " + item.authority);
  }
  return lines.join("\n") + "\n";
}
