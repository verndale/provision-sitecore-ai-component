...
const REASONS = {
  ...
  planEdit:
    "<slug>.plan.json is generated by node src/cli.cjs plan and rewritten on every run; it is part of the step-6 gate review artifact (SKILL.md). Fix the manifest and re-run plan — never hand-edit the plan file.",
  envEdit:
...
function decideFile(filePath, ctx) {
  ...
  if (ctx.inToolRepo && ctx.toolRepoRoot) {
    ...
    if (!rel.startsWith("..")) {
      ...
      if (path.basename(resolved).endsWith(".plan.json")) return deny(REASONS.planEdit);
      return null;
    }
  }
  if (ctx.inProvisioningRepo && path.basename(resolved) === ".env") return deny(REASONS.envEdit);
  // Generated plan artifacts: endsWith keeps goldens (expected-plan.json) and a
  // bare plan.json out of scope while covering any <slug>.plan.json path.
  if (ctx.inProvisioningRepo && path.basename(resolved).endsWith(".plan.json")) return deny(REASONS.planEdit);
  return null;
}
